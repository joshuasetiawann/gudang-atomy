...
export function DashboardLoading() {
  return (
    <div className="space-y-6" role="status" aria-label="Memuat halaman">
      <div className="surface-panel rounded-lg border p-5">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        {Array.from({ length: 6 }).map((_, index) => (
          <div key={index} className="surface-panel rounded-lg border p-5">
    ...
    <div className="overflow-hidden rounded-lg border bg-card shadow-soft">
      ...
      <div className="divide-y">
        {Array.from({ length: rows }).map((_, rowIndex) => (
          <div key={rowIndex} className="grid gap-4 p-4" style={{ gridTemplateColumns: `repeat(${columns}, minmax(92px, 1fr))` }}>
...
export function FormLoading() {
  return (
    <div className="space-y-5" role="status" aria-label="Memuat form">
      <div className="surface-panel rounded-lg border p-5">
...
export function LoginLoading() {
  return (
    <main className="flex min-h-screen items-center justify-center bg-[linear-gradient(180deg,hsl(210_28%_98%),hsl(178_28%_94%))] p-4">
      <div className="surface-panel w-full max-w-md rounded-lg border p-7 shadow-card">
